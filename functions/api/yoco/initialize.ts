...
export async function onRequestPost({ request, env }: FunctionContext) {
  try {
    const { reference, successUrl, cancelUrl, failureUrl } = (await request.json()) as {
      reference?: string;
      successUrl?: string;
      cancelUrl?: string;
      failureUrl?: string;
    ...
    if (!successUrl || !cancelUrl || !failureUrl) {
      return json({ error: "successUrl, cancelUrl, and failureUrl are required" }, 400);
    }
    ...
    await sanityPatch(env, order._id, {
      "yoco.checkoutId": data.id,
      "yoco.redirectUrl": data.redirectUrl,
      "yoco.successUrl": data.successUrl,
      "yoco.cancelUrl": data.cancelUrl,
      "yoco.failureUrl": data.failureUrl,
    });
