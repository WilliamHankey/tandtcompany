...
type Props = {
  product: Pick<Product, "slug" | "name" | "price" | "image"> & { tagline?: string; badge?: string };
  ...
  return (
    <Link to={`/shop/${product.slug}`} className="group block">
      ...
      <div className="mt-4 flex items-baseline justify-between gap-3">
        ...
        <span className="text-sm tabular-nums text-foreground shrink-0">{formatZAR(product.price)}</span>
      </div>
    </Link>
  );
};
